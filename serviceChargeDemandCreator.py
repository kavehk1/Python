...
for row in sheet.iter_rows(min_row=2, values_only=True):
    ...
    run.bold = True
    left_paragraph.add_run(f'{flat_no} Genesis House\n37 Woodlands Road\nSurbiton\nKT6 6PR')
    ...
    footer_para.text = 'Pursuant to the Landlord and Tenant Act 1987 sections 47 and 48 that your Landlord’s name and registered address is GENESIS HOUSE FREEHOLD LIMITED, C/O Virtual Company Secretary Ltd, 7 York Road, Woking, United Kingdom, GU22 7XH England, registration number No: 04533807 and all notices (including notice of proceedings) should be served upon the landlord at their stated registered office address: GENESIS HOUSE FREEHOLD LIMITED C/O Virtual Company Secretary Ltd, 7 York Road, Woking, United Kingdom, GU22 7XH England'


    # first Paragraphs
    firstParagraph = document.add_paragraph()
    firstParagraph.add_run('Invoice for Service Charge Payment\n').bold = True
    firstParagraph.add_run(f'Re: Flat {flat_no}, Genesis House, 37 Woodlands Road, Surbiton, KT6 6PR\n\n\n')
    ...
    tblBorders = parse_xml(
        ...
        r'<w:top w:val="single" w:sz="10" w:space="0" w:color="000000"/>'
        r'<w:left w:val="single" w:sz="10" w:space="0" w:color="000000"/>'
        r'<w:bottom w:val="single" w:sz="10" w:space="0" w:color="000000"/>'
        r'<w:right w:val="single" w:sz="10" w:space="0" w:color="000000"/>'
        r'<w:insideH w:val="single" w:sz="10" w:space="0" w:color="000000"/>'
        r'<w:insideV w:val="single" w:sz="10" w:space="0" w:color="000000"/>'
        r'</w:tblBorders>'
    )
    ...
    run = payPar.add_run('GENESIS HOUSE FREEHOLD LIMITED\n\n')
    run = payPar.add_run(' Bank Details:\n\n').bold = True
    run = payPar.add_run(f'Account Name: GENESIS HOUSE FREEHOLD LIMITED\nAccount Number: 95163107\nSort Code: 04-29-09')

    # 2nd page
    document.add_page_break()

    raoText = """ 1. This summary, which briefly sets out your rights and obligations in relation to variable service charges, must by law accompany a demand for service charges. Unless a summary is sent to you with a demand, you may withhold the service charge. The summary does not give a full interpretation of the law and if you are in doubt about your rights and obligations you should seek independent advice.
    ...
    word_file = (ownerName + '.docx')
    document.save(word_file)
    time.sleep (2)
    convert(word_file, ownerName + '.pdf')
